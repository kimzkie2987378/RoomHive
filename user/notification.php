<?php
/* =========================================================
   ROOMHIVE — NOTIFICATIONS (full page, BOTH roles)
   user/notifications.php

   The bell dropdown's "View all notifications" target.
   Works for hosts AND renters — notifications are keyed by
   user_id, so whoever is logged in sees their own.

   SHELL: role-aware — hosts get the host dashboard sidebar,
   renters get the user account sidebar. Same list markup in
   both (built once into $itemsHtml).

   FEATURES: pagination (20/page), real message text, activity
   icons, listing cover photos for booking/listing links,
   unread highlighting, click = mark read + navigate,
   "Mark all as read".

   === FIXES ===
   1. All navbar/dropdown shell variables now defined BEFORE
      the includes: $navigation, $currentPage, $isHost,
      $isLoggedIn, $userName, $autoOpenLoginPopup.
   2. Viewer SELECT now includes created_at (member_since
      showed the wrong date before).
   3. Host sidebar include guarded with file_exists() so a
      missing host_sidebar.php can't fatal the page.
   4. Renter sidebar highlighted via $activeSidebar.
========================================================= */
...
----------------------------------------------------- */
 $viewerStmt = $pdo->prepare(
    "SELECT id, name, avatar_path, is_host, created_at FROM users WHERE id = :id LIMIT 1"
);
...
if (!$viewer) {
 ...
 $viewerIsHost = !empty($viewer['is_host']);
 $viewerName   = $viewer['name'];
 $navAvatar    = !empty($viewer['avatar_path']) ? $viewer['avatar_path'] : '/webprogg/images/default-avatar.png';

/* =====================================================
   FIX #1 + #3 — SHELL VARIABLES (navbar/dropdown contract)
   Every other page defines these before the includes;
   this page didn't, which is what caused the errors.
===================================================== */
 $isLoggedIn         = true;
 $userName           = $viewerName;
 $isHost             = $viewerIsHost;
 $autoOpenLoginPopup = false;
 $notification_count = 0;

 $navigation = [
    "HOME"          => $viewerIsHost ? "/webprogg/host/hostprofile.php" : "/webprogg/user/usershome.php",
    "LISTINGS"      => "/webprogg/Listings/listing.php",
    "HOW IT WORKS"  => "/webprogg/host/howitworks.php",
    "BECOME A HOST" => $viewerIsHost ? "/webprogg/host/hostprofile.php" : "/webprogg/host/becomeahost.php",
...
/* -----------------------------------------------------
   HELPERS (np_ prefix — cannot clash with the dropdown's)
----------------------------------------------------- */
...
----------------------------------------------------- */
 ...
 $totalStmt = $pdo->prepare("SELECT COUNT(*) FROM notifications WHERE user_id = :u");
...
/* -----------------------------------------------------
   BUILD ITEM MARKUP (shared by both shells)
----------------------------------------------------- */
 $itemsHtml = '';
...
foreach ($rows as $row) {
    ...
    <button
        ...
        data-id="<?php echo $id; ?>"
        data-link="<?php echo h($link !== '' ? $link : '/webprogg/user/notifications.php'); ?>"
    >
        <?php if ($img !== ''): ?>
...
/* -----------------------------------------------------
   SHELL VARIABLES (sidebar contracts)
----------------------------------------------------- */
if ($viewerIsHost) {
    /* host_sidebar.php contract */
    $host = [
        'name'         => $viewerName,
        'avatar'       => $navAvatar,
        /* FIX #2 — created_at is now actually selected,
           so member_since shows the real join date */
        'member_since' => date('F Y', strtotime($viewer['created_at'])),
    ];
    ...
    /* user sidebar.php contract */
    $activeSidebar = 'notifcenter';
}
?>
<!DOCTYPE html>
<html lang="en">
...
<style>
    /* ---- Notifications page (np-*, works in both shells) ---- */
    .np-head {
        display: flex;
        align-items: center;
    ...
    .np-head-sub { margin: 4px 0 0; font-size: 13px; color: #6b7684; }
...
<main class="hp-dashboard hp-dashboard--flush-top">
  <?php
    /* FIX #4 — guard the host sidebar include so a missing
       file can't fatal the whole page */
    if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/webprogg/host/host_sidebar.php')) {
        include $_SERVER['DOCUMENT_ROOT'] . '/webprogg/host/host_sidebar.php';
    }
  ?>
  ...
  <div class="hp-content">
    ...
    <div class="hp-page-header">
      <div>
        ...
        <p class="hp-page-subtitle">Everything that happened on your listings and bookings.</p>
      </div>
    </div>
    ...
    <div class="hp-card">
        ...
        <div class="np-list" id="npList">
            <?php echo $itemsHtml ?: '<div class="np-empty"><span class="np-empty-icon">&#128276;</span>No notifications yet. Actions on your listings will show up here.</div>'; ?>
        </div>

        <?php if ($totalPages > 1): ?>
        <div class="np-pagination">
            <?php if ($page > 1): ?>
                <a class="np-page-btn" href="?page=<?php echo $page - 1; ?>">&larr; Newer</a>
            <?php else: ?>
            ...
            <?php if ($page < $totalPages): ?>
                <a class="np-page-btn" href="?page=<?php echo $page + 1; ?>">Older &rarr;</a>
            <?php else: ?>
...
<main class="up-dashboard">
  ...
  <div class="up-content">
    ...
    <section class="up-card up-reveal in-view">
      ...
      <div class="np-list" id="npList">
        <?php echo $itemsHtml ?: '<div class="np-empty"><span class="np-empty-icon">&#128276;</span>No notifications yet. Activity on your bookings will show up here.</div>'; ?>
      </div>
