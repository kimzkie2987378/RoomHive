...
/* =========================================================
   ...
   KEPT: real time-based greeting, days-as-host, 6-month
   monthly earnings chart + sparkline, occupancy donut,
   spotlight cards, tilt/parallax, 3-dot menu with
   accept/reject/delete.

   CHANGES (this version):
   1. GUARD FIX: non-hosts were redirected to hostprofile.php
      — itself — an infinite loop (the TODO in the old code
      flagged this). They now go to Become a Host.
   2. EDIT WIRED: each listing row gets an "Edit" link to
      /webprogg/user/edit-listing.php — the full editor with
      photo manager. The "someone on the space" lock (price +
      status frozen while a booking is pending/confirmed) is
      enforced inside that page server-side.
...
/* -----------------------------------------------------
   HOST GUARD — FIXED: was a self-redirect (infinite loop)
----------------------------------------------------- */
if (!$dbUser['is_host']) {
...
<section class="hp-hero">
    ...
    <div class="hp-earnings-float" id="hpEarningsFloat">
        ...
        <!-- 6-month sparkline (real monthly data) -->
  ...
  <div class="hp-content">
    ...
    <section class="hp-card hp-spotlight hp-reveal" style="--i: 1;">
      ...
      <div class="hp-stats">
        ...
        <!-- Occupancy as an animated donut ring -->
    ...
    <!-- EARNINGS TREND (real 6-month chart) -->
          ...
          <?php foreach ($listings as $listing): ?>
            <div class="hp-listing-row">
              ...
              <div class="hp-listing-actions">
                ...
                <!-- NEW — Edit wired to the full editor (photo manager
                     + the "someone on the space" price/status lock) -->
                <a href="/webprogg/user/edit-listing.php?id=<?php echo h($listing['id']); ?>"
                   class="hp-btn-outline"
                   style="text-decoration:none; padding:6px 12px; font-size:11.5px;">
                  Edit
                </a>
