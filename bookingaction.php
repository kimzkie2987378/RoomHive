...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: hostbookings.php");
    exit;
}
...
if (!isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $submittedToken)) {
    header("Location: hostbookings.php?msg=error");
    exit;
}
...
if (!$bookingId || !isset($actionToStatus[$action])) {
    header("Location: hostbookings.php?filter={$filter}&msg=error");
    exit;
}
...
header("Location: hostbookings.php?filter={$filter}&msg={$msg}");
exit;
